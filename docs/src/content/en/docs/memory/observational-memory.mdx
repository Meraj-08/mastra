...
Observer and Reflector model calls are retried on transient provider errors, and a turn aborts if they still fail. Two independent options control that on each stage:

- `maxRetries`: retries after the initial model call (default `8`).
- `failurePolicy`: what happens once retries are exhausted — `'abort'` (default) or `'continue'`.
